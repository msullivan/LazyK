...
static inline Expr *newExpr(Type t, Expr *a1 = 0, Expr *a2 = 0) {
...
Expr *cI () { return newExpr(S2, cK(), cK()); }
Expr *KI () { return newExpr(K1, cI()); }

Expr *SI() { return newExpr(S1, cI()); }
Expr *KS () { return newExpr(K1, cS()); }
Expr *KK () { return newExpr(K1, cK()); }
Expr *SKSK () { return newExpr(S2, KS(), cK()); }
...
Expr *make_church_char(int ch) {
	...
	if (cached_church_chars[ch] == 0) {
		if (ch == 0) {
			...
			cached_church_chars[ch] = newExpr(S2, SKSK(), make_church_char(ch-1));
		}
	}
...
static inline Expr *partial_eval_primitive_application(Expr *e, Expr *&prev) {
	...
	case LazyRead: // 6 allocs (4+2 from S2)
	{
		...
		Expr *lhs = e->arg1;
		lhs->type = S2;
		lhs->arg1 = newExpr(S2, cI(), newExpr(K1, make_church_char(getchar())));
		lhs->arg2 = newExpr(K1, newExpr(LazyRead));
		// fall thru
	}
	case S2: // 2 allocs
	{
